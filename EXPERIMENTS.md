...
- **Painting test set (committed):** `Classification == "Paintings"` (exact) = **4,898** dataset classes and **148** of the 1,003 Met test queries (`data/gt_paint/testset.json`) — the single painting definition used project-wide. (An earlier substring/medium exploration counted ~5,143/7,310 classes and 173/221 queries; **dropped for consistency**.) **Val has only ~1–2 painting queries** → can't tune k,τ on val (reuse the full-set K=7, τ=50). `scripts/count_paintings.py`.
...
- **Coverage:** the synthetic renders cover **all 148 committed painting test queries** (`Classification=="Paintings"`) — and the broader exploratory painting set too (+54 generated → all 221). (`scripts/count_paintings.py` + `data/synth_gen/` lists.)
